...
Upgrading arrives in 0.3.4. An install placed by any earlier version can be upgraded in place, and every release after this one arrives the same way. Three regions in your project are upstream-owned and stamped with a version and a body hash — Part 1 of `workflow.md` and the fenced blocks in `CLAUDE.md` / `AGENTS.md`. Everything else is yours: the routine changes nothing outside those three, and the only files it adds to are the `decisions.md` rows and `session-ledger.md` lines that record what it did.

Every install that exists today predates stamped regions, so your first upgrade is the bootstrap walk. To run it: commit your project, then open [CHANGELOG.md](./CHANGELOG.md) and paste the prompt from its **Before 0.3.4** entry — it carries the upgrade rules inline, and they're worth reading before you paste. Later releases each carry their own prompt in their own entry.

Read every diff your AI shows you before saying yes. That reading is the security control — there isn't a stronger one hiding behind it. To back out: `git checkout -- <file>` on the region files, or the backups the routine parks before each write.

The routine itself is [UPGRADING.md](./UPGRADING.md), fetched fresh every time and run under the contract already placed in your `workflow.md`. Every region change arrives as a verbatim diff, nothing is written that you haven't approved, and a region you've edited is reconciled rather than overwritten. What the upgrade defends against — and what it doesn't — is in [SECURITY.md](./SECURITY.md). `allostatik init` still refuses to run on an existing install — upgrading is the path, not re-scaffolding. Today the routine is run by your AI; an `allostatik upgrade` command that does the fetching and classifying for it is planned.
