...
Nothing here writes, and it costs one message. Without it, "recovery is the adopter's commit" names no commit.

- **The session.** If `allostatik/session-ledger.md` doesn't exist, or its last line isn't this session's `OPENED`, run the project's **Session open** step 6 now — append `OPENED s<N> <YYYY-MM-DD>`, creating the file — before anything else. A bootstrap has no ledger, and a kickoff prompt doesn't mention one. So a project's whole record begins here. *First run* step 0 in `workflow.md` prevents the same omission on the install path, and says why: skip it and the close's own check fails by construction on that project's first session. Take `s<N>` from the ledger; with no ledger, from the newest entry in `log.md` — or `plan.md`'s session log on a pre-0.3.5 install — and state which you used rather than assuming it.
...
These become tokens on the step-1 line; nothing is appended yet.
...
5. **How to stop** — nothing is written until they say go. Then the backups, and the commit step 0 recorded.
...
- **UNMARKED** (the bootstrap) — there is no stamp to compare against. Show current → reference and walk it per the PRISTINE-STALE rules, section by section. Anything the adopter keeps that differs from the reference gets a blessing row, as for CUSTOMIZED — with `base` set to `pre-stamp` here, not a version. Legacy fence markers without a version and hash are replaced by the reference's marker lines. For `part1`, the END marker goes above Part 2's opening line, with no heading and no content between them. Blank lines and a horizontal rule are permitted. Part 2's opener is the line that *begins* with `**Part 2 —`, matched at the start of a line — never the mention of that same string inside Part 1's own preamble. Never at end of file. The bootstrap ends **stamped**.
