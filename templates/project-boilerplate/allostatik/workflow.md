...
5. **Confirm it took.** Start a fresh conversation and check Claude orients from the files. *Check (output):* the files carry real project content, placeholders are filled, and the pointer is deployed. *If it fails:* finish the missing piece before treating setup as done. Then append `STEP-DONE first-run` to the ledger, so a later session reads setup as finished rather than abandoned.
...
6. **Deploy the pointer and confirm it took** — same as the fresh-project routine, steps 4–5. Then append `STEP-DONE migrate` to the ledger, so a later session reads the migration as finished rather than abandoned.
