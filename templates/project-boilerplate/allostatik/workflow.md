<!-- BEGIN allostatik-part1 v0.3.6 sha256:0a38ca66d844 -->
# Workflow
...
2. **Everything is shown before it's written.** A region change as a **verbatim diff** that includes the two marker lines; a `decisions.md` row or ledger line as its exact text. One region at a time, approval-then-apply as the atomic unit. A summary never substitutes for the diff; diffs are shown in fenced blocks so comments and markup stay visible; a changed line longer than 200 characters is shown word-by-word, not line-by-line; a diff longer than 60 changed lines or touching more than one `##` section is walked section by section. Before a walked diff, state what the change is **for** — the release's own words for it — and which sections carry that one intent; sections approved without it are fragments approved without meaning, which is consent in form only.
