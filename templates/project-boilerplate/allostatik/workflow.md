...
8. **Name the session and confirm it's done.** Suggest a short, descriptive name for this session so the conversation is easy to find later, then confirm the session is complete for now. A format that scans well: `s<N> <project> <what shipped>` — session number, project shorthand, then an echo of the session's commit(s), or of the work done when nothing was committed; the number and shorthand keep sessions findable across projects, and the commit echo makes the conversation list read as a changelog. *Check (output):* a name is offered and the wrap is acknowledged.
