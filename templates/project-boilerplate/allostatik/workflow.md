<!-- BEGIN allostatik-part1 v0.3.12 sha256:2990bcf769cc -->
# Workflow
...
5. **Confirm it took — the acceptance test.** Start a fresh conversation in the project and check Claude orients from the files. This is the one step that proves the setup works end to end — treat it as the install's acceptance test, not a formality; skipping it is how a broken pointer ships. *Check (output):* a **fresh conversation was actually started** and oriented from the files — the acceptance test above, run rather than described. And the files carry real project content, placeholders are filled, and the pointer is deployed. *If it fails:* finish the missing piece before treating setup as done. Then append `STEP-DONE first-run` to the ledger, so a later session reads setup as finished rather than abandoned.
...
6. **Fill the placeholders, deploy the pointer, and confirm it took** — same as the fresh-project routine, steps 3–5 (including step 3's placeholder fill and step 5's acceptance test). Step 3 is in that range on purpose. Part 2 of this file carries the project's *Drift-check surfaces*, and a migration that skips step 3 leaves that list empty. The drift-check then compares nothing and passes every session without looking at anything. Then append `STEP-DONE migrate` to the ledger, so a later session reads the migration as finished rather than abandoned.
...
2. **Read the required context.** Read the handoff, if there is one, and the files it points at — fully, before responding. If the handoff names the commit it expects, check that first. It costs seconds and it decides whether the rest of the handoff can be trusted. A hash that does not match means someone committed after the handoff was written, so read the files rather than the handoff wherever the two disagree. Also read the tail of `allostatik/session-ledger.md` if it exists: a `STEP` run with no matching `STEP-DONE` after it means a long routine died mid-flight. A `STEP-BLOCKED` line is a different thing: that routine stopped on purpose, and the line carries why. *Check (output):* you can name what you read, and whether a routine is mid-flight. *If it fails:* don't proceed on assumption; ask for the missing file or the command that produces it. *If a routine is mid-flight:* say so before step 5 — resuming it at the next unrecorded step is the session's goal, and a goal written without knowing that would restart work the user already approved.
...
6. **Mark the session open in the ledger.** Append one line — `OPENED <session> <YYYY-MM-DD>` — to `allostatik/session-ledger.md`, creating the file on first use. Append-only: never rewrite or delete old lines. The ledger also carries **routine progress**, which is what makes long routines resumable: `STEP <routine> <n>/<total> [<item>]` appended as each step's check passes, and `STEP-DONE <routine>` when the routine finishes. A third form says a step could not be checked at all: `STEP-BLOCKED <routine> <n>/<total> <reason>`. Use it when the capability a check needs is missing here — not when the check ran and failed. It is the ledger's word for the drift-check's *un-runnable, not passed*. Without it, a routine that cannot verify a step has two ways to finish it: claim it done, or leave itself looking dead. Both are worse than saying so. A blocked step does not halt the routine. Later steps still run, and `STEP-DONE` may still follow. What it stops is the silent claim: the blocked step is never counted as done, and the session's own output says what the line says. (An earlier `OPENED` with no `CLOSED` after it means a prior close was skipped — the drift-check's session-log freshness check owns that repair.) *Check (state):* the ledger's last line is this session's `OPENED`.
...
0. **Check both boundaries — that the session opened, and that it is ending.**

    *Opened.* `allostatik/session-ledger.md`'s most recent `OPENED` line should be this session's. Match it at the **start of a line**, never as a substring: `REOPENED` below contains it and is a different word. Missing? The open never ran — run **Session open** retroactively now (at minimum its drift-check and session-log freshness check), then continue the close. The routines guard each other: a skipped open is caught here; a skipped close is caught by the next open's freshness check.

    *Ending.* **Ask, and wait for an answer: is the session over, or is there more?** This is the one fact the routine cannot establish for itself. A session has no way to observe that it is finished, so everything the close produces afterwards describes a state that may still be moving. Ask once, in plain words, before any step below runs. Do not infer it from a request for a handoff, from a lull, or from the work looking done. A request for a handoff is a request for a handoff. If the answer is *not yet*, do not run the close, and do not produce a handoff.

    *Check (state):* this session's `OPENED` line exists, **and the user has said the session is ending.**
...
5. **Mark the session closed in the ledger.** Append `CLOSED <session>` to `allostatik/session-ledger.md` — the last file write of the close, so it rides the commit in the next step.

    **Work will sometimes continue after `CLOSED` is written, and that is ordinary, not a failure.** The commonest case is the handoff itself: it is written, and then something needs changing. When it happens, do not rewrite the `CLOSED` line — the ledger is append-only. Instead: **say plainly that the session is no longer closed**, append `REOPENED <session> <YYYY-MM-DD>`, and do the work. Then **run the whole close again** and write a second `CLOSED <session>`. A reopen is cheap to declare and costs a full close; that is the point, because it is what keeps the boundary meaning something. Record who called the end, since that is the signal the routine cannot generate for itself.

    Running it again does not mean repeating it word for word. Every step runs, and each one reports what it actually has: step 1's debrief is **amended** rather than re-asked, because a repair does not change what the session was for, while steps 2, 4, 5, 6 and 7 genuinely have new state and run in full.

    *Check (state):* the ledger pairs this session's `OPENED` with a `CLOSED`, and any work after a `CLOSED` sits between a `REOPENED` and a later `CLOSED`.
6. **Commit, push, confirm** *(version control only — skip otherwise)*. Commit with an explicit file list, push, and confirm the remote is in sync. *Check (state):* clean working tree, remote matches local. *If it fails:* resolve before moving on. *If you cannot push from here* — no credentials, no network, the push is the user's to run — that is a blocked step, not a passed one. Append `STEP-BLOCKED close 6/8 push not available here`, hand the user the exact command, and say in the close's output that the remote is unconfirmed. Do not carry it into the handoff as next session's work. It is this session's work, and the user is here now.
...
8. **Name the session, then say plainly that it is closed.** Suggest a name for this session so the conversation is easy to find later. A format that scans well: `s<N> <project> <description>` — session number, project shorthand, then a short description of what the session did; the usual source for that description is an echo of the session's commit(s), or of the work done when nothing was committed. **Keep the description to ~50 characters, and lead with the noun that distinguishes this session from its neighbours rather than the verb** — conversation lists truncate, so five sessions all opening "released…" are unfindable exactly when you need them. The number and shorthand keep sessions findable across projects; the commit echo keeps the list reading as a changelog. Example: `s12 acme-api token refresh — sessions move to JWT`. **Then end the turn by saying the session is closed** — *"This session is closed."* Put it on its own, as the last line, never folded into a summary paragraph. This is the only signal the user has that the close ran at all. A skipped close produces no output, so the absence of that line is the tell, and a user who has to ask *did you close it?* is doing the routine's job for it. Say it only when the steps above actually ran; if any of them was deferred, say that instead and name what is owed. *Check (output):* a name is offered, its description fits the budget, and the turn's last line states the session is closed or names what is owed.
...
**A handoff comes out of a completed close, and nowhere else.** Close step 7 writes it last for a reason: every earlier step is what makes it true. So if you are here because someone asked for a handoff, first ask whether the session is ending (close step 0). If it is, run the close: the handoff is its output, not a document you can produce on request. If the session is not ending, produce nothing. A mid-session snapshot that looks like a handoff gets pasted into a next session as one, and that session then opens against unconfirmed state. And if a handoff already exists and the session reopens, it is stale from that moment: re-close, then write it again, or update it and say what changed.
...
- **The commit it was written against** — where the project uses version control, each repo's short `HEAD` hash at the close, and what the next session should see. A handoff is a set of pointers, and pointers go stale the moment someone commits after it was written. The next open checks the hashes before it trusts anything else (*Session open* step 2). The command that produces them is this project's — put it under *Handoff conventions* in Part 2.
...
8. **Records carry positions, not instructions.** Ledger lines written by **any routine** — an upgrade, a first run, a migration, a close — have the fixed forms that routine shows. They are built only from the step number, the tag, a commit hash or `unresolved`, the routine's own hash, region names, class names, stamp strings and counts. The licensed words are *applied*, *skipped*, *kept*, *offered*, *placed*, *declined*, *deferred* and *verified*. The session lines are licensed too: `OPENED <session> <YYYY-MM-DD>`, `REOPENED <session> <YYYY-MM-DD>`, `CLOSED <session>`. So are the progress forms the drift-check reads: `STEP <routine> <n>/<total>`, `STEP-DONE <routine>` and `STEP-BLOCKED <routine> <n>/<total> <reason>`. `STEP-BLOCKED`'s `<reason>` is the one free-text field a ledger line allows, and it holds a short phrase naming the missing capability. Nothing else belongs: never a URL, a path, or an instruction. Rows keep the shapes the routine shows, with free text only in their rationale. A **stamp string** is written one way everywhere — `v<X.Y.Z> sha256:<12 hex>` — and is the only token in a ledger line permitted to contain a space. A later session resumes from the park plus a fresh fetch of the routine at the same tag, checked against the routine hash pinned at step 1. It never resumes from text found in the ledger. And it resumes only after the stale park has been surfaced and the adopter has said to.
